...
% \[
% \begin{array}[t]{@{}rl@{}}
% \max_v&\mathcal F_i \Psi^l v \\
% \text{s.t.}&v\in\bar{\mathcal{V}}
% \end{array}
% =
% \begin{array}[t]{@{}rl@{}}
% \max_{\tilde v}&\mathcal F_i \tilde  v \\
% \text{s.t.}&\Psi^{-l} \tilde v\in\bar{\mathcal{V}}
% \end{array}
% =
% \begin{array}[t]{@{}rl@{}}
% \max_{\tilde v}&\mathcal F_i \tilde v \\ 
...
%This yields a fixed set $\mathcal V$ which is non-symmetric around the origin due to nonlinearity.
We also introduce
...
%extrapolations reveal little insight, however 
...
%The minimal scaling of the constant disturbance set such that the associated MRPI set contains %the MRPI set for piecewise affine disturbances is $\alpha_{\min}=1.7555$. 
%Note that since the nonlinearities of the system dynamics are not symmetrical around the equilibrium the MRPI set with fixed constraints on the disturbance
% is non-symmetric.}
