...
Now consider a parametrically convex set of the form of~\eqref{eq:PWA:disturbance:set}.
    ...
    \end{thm}
    % \begin{thm}\label{thm:convex:parametric:set}
    %   The point-to-set map $\mathcal V (x)$ defined by~\eqref{eq:PWA:disturbance:set} for a function $H(x)$ that is elementwise convex in $x\in \mathcal X\subset \mathbb R^n$ (i.e.~each element $H_i(x)$ of $H(x)$ satisfies
...
>>>>>>> a43e026728e0dd64d7ee69126688533887ca39de
...
%The case that  $N<n$ can occur if $\mathrm{rank}(\Gamma) > 1$.
%More generally, to 
...
%+ A \tilde x + B \tilde u - A \tilde x - B\tilde u\\
\Leftrightarrow 
...
%In the next section we will illustrate the algorithm.
%
%
%
%
...
%Using the same constraint set $\mathcal X \times \mathcal U$ to determine 
...
% , in fact it is easy to see that both signs are likely to be 
