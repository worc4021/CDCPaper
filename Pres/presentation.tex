...
\setbeamercolor{plots}{fg=black,bg=red}
...
\begin{tikzpicture}[scale=1.5]
	...
	\only<1-2>{
	\draw[-latex'] (-3,1.5) to[out=0,in=180] (-2,1.1);
	\draw (-3,1.5) node[left] {{$S$}};}
	\only<2->{
	\draw[fg] plot[domain=-2:2] (\x, {1.8 -.1*pow(\x,2))});}
	\only<2>{
	\draw[-latex'] (0,.2) to[out=45,in=-160] (1.2,1.656);
	\draw (0,0) node[bottom,rotate=0] {{$\{x: \exists t\in T(x)\; t + x\in \partial S\}$}};
